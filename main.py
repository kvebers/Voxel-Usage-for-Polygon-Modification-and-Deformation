...
def cameraHelper(cam_keys, sim_time):
...
def gameLoop(config, cam_keys, ground_y, duration, scene_objects, shader):
    ...
    frames = config.get('simulation', {}).get('maxFrames', 120)
    while running:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                running = False
        dt = clock.tick(frames) / 1000.0
        dt = min(dt, 0.1)
        sim_time += dt
        if duration > 0.0 and sim_time >= duration:
            running = False
            continue
        cameraHelper(cam_keys=cam_keys, sim_time=sim_time)
        draw_floor(ground_y)

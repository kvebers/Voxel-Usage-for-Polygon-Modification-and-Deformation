...
def health_to_colors(health_values):
    """Turn health 0.0-1.0 into RGB color. Dead=red, hurt=orange/yellow, healthy=blue/green."""
    ...
    mask = health_values <= 0.0  # disconnected
    r[mask], g[mask], b[mask] = 0.9, 0.15, 0.1
    mask = (health_values > 0.0) & (health_values <= 0.25)  # critical
    ...
    mask = (health_values > 0.25) & (health_values <= 0.5)  # hurt
    ...
    mask = (health_values > 0.5) & (health_values <= 0.75)  # recover
    ...
    mask = health_values > 0.75  # healthy
...
class JointBreaker:
    """
    Tracks damage on every joint between voxels.
    Breaks joints when stress is too high.
    Colors health
    """
    ...
    def __init__(self, scene, model, angular_break_torque=5e5, damage_rate=0.4, heal_rate=0.05, instant_break_torque=2e6, max_breaks_per_step=5):
        """
        Set up damage tracking for all joints in the scene.
        """
        self.neighbor_pairs = scene["neighbor_pairs"]
        self.positions = scene["positions"]
        self.vbs = scene["voxel_body_start"]  # index offset so we find the right body in the model
        self.voxel_count = scene["voxel_count"]
        self.solver = scene["solver"]
        joint_count = len(self.neighbor_pairs)
        self.damage = np.zeros(joint_count, np.float32)  # 0=healthy, 1=broken
        self.broken = np.zeros(joint_count, dtype=bool)  # True=joint removed
        self.angular_break_torque = angular_break_torque  # stress level that accumulates damage
        self.damage_rate = damage_rate  # how fast damage builds up
        self.heal_rate = heal_rate  # how fast damage repairs itself
        self.instant_break_torque = instant_break_torque  # stress level that breaks joint immediately
        self.enabled = joint_count > 0 and model.joint_count >= joint_count  # skip if no joints
        self.max_counts = np.zeros(self.voxel_count, np.int32)  # max possible joints per voxel
        if not self.enabled:
            return
        self.init_joint_tracking(model, joint_count, max_breaks_per_step, scene)

    def init_joint_tracking(self, model, n, max_breaks_per_step, scene):
        """
        Build lookup tables so we can quickly find which joints belong to which voxels.
        """
        if "joint_start" in scene:
            self.joint_enabled_offset = scene["joint_start"]  # where our joints start in the model list
        else:
            self.joint_enabled_offset = model.joint_count - n  # assume our joints are at the end
        self._constraint_start = self.solver.joint_constraint_start.numpy().copy()  # which constraint index each joint starts at
        self._constraint_dim = self.solver.joint_constraint_dim.numpy().copy()  # how many constraints each joint has
        for voxel_a, voxel_b in self.neighbor_pairs:
            self.max_counts[voxel_a] += 1  # count max joints each voxel can have
            self.max_counts[voxel_b] += 1
        self._voxel_joints = [[] for _ in range(self.voxel_count)]  # joint list per voxel
        for joint_index, (voxel_a, voxel_b) in enumerate(self.neighbor_pairs):
            self._voxel_joints[voxel_a].append(joint_index)  # voxel knows which joints it owns
        ...
        self._parent_body_indices = self.vbs + pairs[:, 0]  # actual body index in model for parent side
        self._child_body_indices = self.vbs + pairs[:, 1]  # actual body index in model for child side
        self._joint_enabled_cache = model.joint_enabled.numpy().copy()  # local copy to avoid GPU reads every frame

    def get_voxel_colors(self):
        """
        Return RGB color per voxel based on how many joints it still has alive.
        """
        pairs = np.array(self.neighbor_pairs, dtype=np.int32).reshape(-1, 2)
        active = ~self.broken
        counts = np.zeros(self.voxel_count, np.int32)
        np.add.at(counts, pairs[active, 0], 1)  # count alive joints per voxel
        np.add.at(counts, pairs[active, 1], 1)
        safe_max = np.maximum(self.max_counts, 1)  # avoid divide by zero
        t = np.clip(counts.astype(np.float32) / safe_max, 0.0, 1.0)  # health 0-1 from joint count
        return health_to_colors(t)

    def compute_damage(self, body_torques, dt):
        """
        Calculate stress on each joint and update damage.
        Return mask of instantly broken joints.
        """
        diff = body_torques[self._parent_body_indices] - body_torques[self._child_body_indices]  # torque difference across joint
        ang_stress = np.linalg.norm(diff, axis=1)  # scalar stress per joint
        instant = ang_stress > self.instant_break_torque  # joints that snap immediately
        norm_stress = np.maximum(0.0, ang_stress / max(self.angular_break_torque, 1e-8) - 0.5)  # normalized stress, 0 below threshold
        active = ~self.broken & ~instant
        # under stress: accumulate damage; no stress: slowly heal
    ...
    def get_newly_broken(self, instant):
        """
        Find joints that just broke this frame.
        Cap at max_breaks_per_step, to not have explosions.
        """
        joint_count = len(self.neighbor_pairs)
        newly_broken = (~self.broken) & (instant | (self.damage >= 1.0))  # not already broken AND now broken
        newly_broken_indices = np.where(newly_broken)[0]
        if len(newly_broken_indices) > self._max_breaks_per_step:
            newly_broken_indices = newly_broken_indices[: self._max_breaks_per_step]  # limit breaks per frame
    ...
    def apply_breaks(self, model, newly_broken, newly_broken_indices, joint_enabled, device):
        """
        Mark joints as broken and disable them in the physics model.
        """
        ...
        for joint_index in newly_broken_indices:
            ...
            if model_index < len(joint_enabled):
                joint_enabled[model_index] = 0  # tell physics engine joint not connected
        self.flush_to_solver(model, joint_enabled, device)

    def update(self, model, dt, device, body_torques=None):
        """
        Main update called every frame.
        Compute damage, find breaks, apply them.
        """
        if not self.enabled:
            return
        joint_enabled = self._joint_enabled_cache
        if body_torques is None:
            body_torques = self.solver.body_torques.numpy()  # read torques from GPU
    ...
    def mark_broken(self, ji, joint_enabled):
        """
        Force-break a specific joint by index.
        """
        self.broken[ji] = True
        self.damage[ji] = 1.0
    ...
    def get_state_data(self):
        """
        Read current solver state into CPU arrays.
        """
        solver = self.solver
        return (
    ...
    def no_broken_penalties(self, penalty_k, penalty_k_max, penalty_kd, penalty_k_min):
        """
        Zero out penalty springs for broken joints.
        Stop from solving.
        """
        for ji in range(len(self.neighbor_pairs)):
            if not self.broken[ji]:
                continue
            model_index = self.joint_enabled_offset + ji
            c_start = int(self._constraint_start[model_index])  # first constraint
            c_dim = int(self._constraint_dim[model_index])  # how many constraints
            for c in range(c_start, c_start + c_dim):
                if c < len(penalty_k):
                    penalty_k[c] = 0.0  # no stiffness
                    penalty_k_max[c] = 0.0
                    penalty_kd[c] = 0.0  # no damping
                    penalty_k_min[c] = 0.0

    def no_broken_state(self, sigma_prev, sigma_start, kappa_prev, dkappa_prev, c_fric):  # TODO remove in diff patch
        """
        Zero out solver internal state for broken joints. Bug fix.
        """
        zero3 = (0.0, 0.0, 0.0)
        for ji in range(len(self.neighbor_pairs)):
            if not self.broken[ji]:
    ...
    def sync_state_arrays(self, sigma_prev, sigma_start, kappa_prev, dkappa_prev, c_fric, device):
        """
        Push updated constraint state arrays back to GPU.
        """
        solver = self.solver
    ...
    def flush_to_solver(self, model, joint_enabled, device):
        """
        Push all break changes to GPU so physics engine sees them next step.
        """
        ...
        try:
            self.solver.notify_model_changed(None)  # tell solver something changed
        except Exception:
            pass

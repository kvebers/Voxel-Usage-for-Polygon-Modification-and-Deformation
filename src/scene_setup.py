...
def get_object_configs(cfg):
    ...
    return [SimpleNamespace(path=cfg.mesh.path, offset=[0.0, 0.0, 0.0], color=list(cfg.render.mesh_color), resolution=cfg.mesh.resolution)]
...
def load_and_voxelize_one(cfg, obj_config, ctx=None):
    ...
    (_, _, _, coords, coord_to_index, neighbor_pairs, block_halves_voxel, joint_voxel_offsets) = voxelize_and_build_topology(
        centered_verts, indices, obj_config.resolution, fill_mode=fill_mode, ensure_connected=ensure_connected, greedy_merge=greedy_merge, ctx=ctx
    )
    return {
...
def scale_greedy_to_world(block_halves_voxel, joint_voxel_offsets, voxel_size):
    ...
    if block_halves_voxel is not None:
        block_halves_world = [(half_x * voxel_size, half_z * voxel_size, half_y * voxel_size) for half_x, half_y, half_z in block_halves_voxel]
    if joint_voxel_offsets is not None:
        joint_world_offsets = [
            (
                (parent_offset[0] * voxel_size, -parent_offset[2] * voxel_size, parent_offset[1] * voxel_size),
                (child_offset[0] * voxel_size, -child_offset[2] * voxel_size, child_offset[1] * voxel_size),
            )

...
def load_obj(path: str):
    ...
    if v_parts:
        ...
        vertices = raw.reshape(len(v_parts), n_col)[:, :3].copy()
    else:
    ...
    use_fast = (
        bool(f_parts)
        and "/" not in f_parts[0]
        and (not f_vcounts or f_vcounts[0] == n_verts)
    )
    ...
    if not f_parts:
        ...
        indices = np.array(idx_list, dtype=np.int32)
    if len(vertices):
    ...
    if len(indices) >= 3:
        ...
        indices = tri[no_dup & no_degen].reshape(-1)
